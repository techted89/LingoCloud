...
public class TranslationBridgeTest {
    ...
    public void testRequestTranslation_cachedHit() {
        ...
        try (MockedStatic<HookMain.TranslationCache> mockStatic = mockStatic(HookMain.TranslationCache.class)) {
            mockStatic.when(() -> HookMain.TranslationCache.get(originalText)).thenReturn(translatedText);
